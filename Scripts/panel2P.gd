...
# TaskManagers
@onready var task_manager_one = $TaskManager
...
func _ready():
	...
	# Réinitialisation de l'opacité des croix
	croix1.modulate.a = 0
	croix2.modulate.a = 0
	croix3.modulate.a = 0
	
	# ----------------------- Liaison avec les TaskManagers ----------------------- #
	task_manager_one.task_completed.connect(_on_task_completed)
	task_manager_one.task_failed.connect(_on_task_failed)

	task_manager_two.task_completed.connect(_on_task_completed)
	task_manager_two.task_failed.connect(_on_task_failed)
...
# ----------------------- Gestion du Timer et des Labels ----------------------- #
...
func _on_task_failed(_task_id: String):
	...
	# Calcul du total d'erreurs combinées des deux TaskManagers
	var total_errors = task_manager_one.get_failed_tasks_count() + task_manager_two.get_failed_tasks_count()
